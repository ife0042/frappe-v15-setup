## Frappe v15 Setup Scripts

These scripts automate a two-phase provisioning flow for a Frappe v15 development/test environment on Ubuntu 24.04 LTS (Noble) only. They separate privileged, system-level tasks from user-level app setup for better security and repeatability, and provide a single-command orchestrator for convenience.
...
- **`setup-frappe-app.sh`**: Non-root, app user setup.
  ...
  - Installs wkhtmltopdf for Ubuntu 24.04 (Noble), Node via NVM, Yarn, Python/pip, Bench CLI.
...
- **Network**: Outbound internet access (downloads `nvm`, wkhtmltopdf, Bench).
...
```bash
sudo mkdir -p /opt/frappe-v15-setup
# Recommended: clone directly on the server into /opt/frappe-v15-setup
sudo git clone https://github.com/ife0042/frappe-v15-setup /opt/frappe-v15-setup
sudo chmod +x /opt/frappe-v15-setup/*.sh
```
...
```bash
sudo /opt/frappe-v15-setup/setup-orchestrator.sh \
  -u frappe \
...
```bash
sudo /opt/frappe-v15-setup/root-system-setup.sh \
  -u frappe \
  -k "ssh-ed25519 AAAA... user@example.com"
```
...
```bash
/opt/frappe-v15-setup/setup-frappe-app.sh \
  -u frappe \
  -p 'mariadb_root_password' \
  -s 'apps.localhost' \
  -a 'frappe_admin_password' \
...
- **NVM + Node 18 + Yarn**: Frappe asset builds rely on Node/Yarn.
- **Python 3 + pip**: Used to install Bench.
- **Bench CLI (5.x)**: Official tool to manage Frappe benches.
- Bench init with `--frappe-branch version-15` and site creation.
